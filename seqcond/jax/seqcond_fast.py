...
class SeqCondAttention(nn.Module):
    ...
    num_anchor_heads: int = 0
    num_thetas: int = 1  # FAST path here assumes M=1
    ...
    # streaming chunk over sequence length
    chunk_size: int = 128
    ...
    def __call__(self, x, mask=None, deterministic=True):
        ...
        log_time_weight = (
            jnp.concatenate(log_w_list, axis=2) if log_w_list else jnp.zeros((1, l, self.K, 1), jnp.float32)
        )
        score_scale = self.param("score_scale", nn.initializers.ones, (self.K,))
