...
def p_orthogonal_(key, blocks, rows, cols, gain, dtype):
    ...
    return jax.vmap(inner_orthogonal, in_axes=(0, None, None, None))(
        flattened, rows, cols, gain
    ).astype(dtype)
...
class BaseRWKV(LLM):
    ...
    def randomize_att(cls, att_key, n_layer, n_embd, vocab_size, dtype):
        ...
        www = -6 + 6 * jnp.pow(
            jnp.arange(n_embd, dtype=dtype)[None] / (n_embd - 1),
            1 + 1 * ratio_0_to_1[:, None] ** 0.3,
        )
        ...
        orthos = jnp.astype(
            jax.random.orthogonal(att_key, n_embd, shape=(4, n_layer)), dtype
        )
        ...
        params["ln_x"] = {
            "bias": jnp.zeros((n_layer, n_embd), dtype=dtype),
            "weight": jnp.astype(
                jnp.repeat(
                    ((1 + jnp.arange(n_layer)) / n_layer)[:, None] ** 0.7,
                    n_embd,
                    axis=1,
                ),
                dtype,
    ...
    def randomize_ffn(cls, ffn_key, n_layer, n_embd, vocab_size, dtype):
        ...
        params["key"] = {
            "weight": p_orthogonal_(ffn_key, n_layer, hidden_sz, n_embd, 1.0, dtype)
        }
        params["value"] = {
            "weight": jnp.zeros((n_layer, n_embd, hidden_sz), dtype=dtype)
        }
    ...
    def randomize_weights(cls, key, n_layer, n_embd, vocab_size, config, dtype):
        ...
        emb_scale = 1e-4
        head_scale = (
            0.5 if vocab_size <= n_embd else 0.5 * jnp.sqrt(vocab_size / n_embd)
        )
        params = {
            "blocks": {
                ...
                "ln1": {
                    "bias": jnp.zeros((n_layer, n_embd), dtype=dtype),
                    "weight": jnp.ones((n_layer, n_embd), dtype=dtype),
                },
                "ln2": {
                    "bias": jnp.zeros((n_layer, n_embd), dtype=dtype),
                    "weight": jnp.ones((n_layer, n_embd), dtype=dtype),
            ...
            "head": {
                "weight": orthogonal_(head_key, vocab_size, n_embd, head_scale, dtype)
            },
            "ln0": {
                "bias": jnp.zeros(n_embd, dtype=dtype),
                "weight": jnp.ones(n_embd, dtype=dtype),
            },
            "ln_out": {
                "bias": jnp.zeros(n_embd, dtype=dtype),
                "weight": jnp.ones(n_embd, dtype=dtype),
    ...
    def default_state(cls, params, config):
        ...
        return jnp.zeros(
            (n_layer, (2 + head_size), n_embd), dtype=params["emb"]["weight"].dtype
        )
    ...
    @classmethod
    def time_mixing_seq(
        cls, x, state, v_first, att, length, new_starts, H, S, layer_id
    ):
        T, C = x.shape
        ...
        v = jnp.where(
            ...
            v,
            v
            + (v_first - v) * jax.nn.sigmoid(att["v0"] + (xv @ att["v1"]) @ att["v2"]),
        )
        ...
        r, w, k, v, a_i, b_i = tuple(
            [val.reshape(T, H, S) for val in (r, w, k, v, -kk, kk * a)]
        )
        ...
        x = group_norm(
            x,
            num_groups=H,
            weight=att["ln_x"]["weight"],
            bias=att["ln_x"]["bias"],
            eps=64e-5,
        )
        x = x + (
            jnp.sum(
                r.reshape(1, T, H, -1) * k.reshape(1, T, H, -1) * att["r_k"],
                axis=-1,
                keepdims=True,
            )
            * v.reshape(1, T, H, -1)
    ...
    def forward_seq(cls, params, config, x, state, length, new_starts):
        ...
        def block_loop(y, inputs):
            ...
            x_new, s, v_first = cls.time_mixing_seq(
                layer_norm(x, block["ln1"]),
                state[1:],
                v_first,
                block["att"],
                length,
            ...
            x_new, s = cls.channel_mixing_seq(
                layer_norm(x, block["ln2"]), state[:1], block["ffn"], length, new_starts
            )
        ...
        (x, _), state = jax.lax.scan(
            block_loop, (x, v_first), (params["blocks"], state, jnp.arange(n_layer))
        )
        return x, state
...
class ScanRWKV(BaseRWKV):
    ...
    def inner_loop(cls, r, w, k, v, a, b, s, length, new_starts):
        ...
        def scan_loop(inner_states, x):
            ...
            sa = inner_state @ at
            inner_state = jnp.astype(
                inner_state * wt + vt @ kt + sa @ bt, inner_state.dtype
            )
        ...
        (s, _), out = jax.lax.scan(
            scan_loop, (s, s), (r, w, k, v, a, b, idxes), unroll=64
        )
...
class AssociativeScanRWKV(BaseRWKV):
    ...
    def inner_loop(cls, r, w, k, v, a, b, s, length, new_starts):
        ...
        merge_fn = jax.vmap(
            jax.vmap((lambda wi, ai, bi: jnp.diag(wi) + jnp.outer(ai, bi)))
        )
        ...
        new_starts = jnp.concatenate((new_starts, jnp.zeros_like(new_starts[:1])))[
            :, None, None, None
        ]
...
def batched_wkv7_fwd(r, w, k, v, a, b, s, length, new_starts, B):
    ...
    out, s_restore, sa_restore, new_s = jax.ffi.ffi_call(
        f"wkv7-fwd-{dtype_str}", (out_type, s_restore_type, sa_restore_type, s_type)
    )(
        w,
        r,
        k,
...
def batched_wkv7_bwd(res, grads, B):
    ...
    if DO_BACKWARDS_WARNING:
        ...
        print(
            "WARNING: RWKV7 Cuda Kernels do not currently receive gradients from the output state or send gradients to the input state, making it unsuitable for state tuning. Gradients will also explode if you provided new_starts. Double check your results against AssociativeScanRWKV for correctness."
        )
        DO_BACKWARDS_WARNING = False

    assert (
        T % 16 == 0
    ), f"Must have sequence length ({T}) equal to a multiple of the chunk length (16)"

    gw, gr, gk, gv, ga, gb, gs = jax.ffi.ffi_call(
        f"wkv7-bwd-{dtype_str}",
        (gw_type, gr_type, gk_type, gv_type, ga_type, gb_type, gs_type),
    )(
        w,
        r,
        k,
...
def wkv7_fwd_vmap_rule(axis_size, in_batched, r, w, k, v, a, b, s, length, new_starts):
    ...
    return batched_wkv7_fwd(
        r, w, k, v, a, b, s, jnp.uint32(length), new_starts, k.shape[0]
    ), ((True, True), tuple(in_batched) + (True, True))
    ...
    def get_kernels(cls):
        ...
        if UNLOADED_KERNEL:
            ...
            SHARED_LIBRARY = os.path.join(
                os.path.dirname(jaxrwkvkernel.__file__), "lib_wkv7.so"
            )
            library = ctypes.cdll.LoadLibrary(SHARED_LIBRARY)
            jax.ffi.register_ffi_target(
                "wkv7-fwd-fp32", jax.ffi.pycapsule(library.WKV7FwdF32), platform="CUDA"
            )
            jax.ffi.register_ffi_target(
                "wkv7-fwd-bf16", jax.ffi.pycapsule(library.WKV7FwdBF16), platform="CUDA"
            )

            jax.ffi.register_ffi_target(
                "wkv7-bwd-fp32", jax.ffi.pycapsule(library.WKV7BwdF32), platform="CUDA"
            )
            jax.ffi.register_ffi_target(
                "wkv7-bwd-bf16", jax.ffi.pycapsule(library.WKV7BwdBF16), platform="CUDA"
            )
