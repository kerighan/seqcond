...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Count SeqCond model parameters",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Step to resume training from (Ignored)",
    )
    parser.add_argument(
        "--resume-checkpoint",
        type=str,
        default=None,
        help="Specific checkpoint file path (Ignored)",
    )
    ...
    grp_model.add_argument(
        ...
        help="Use square matrix for seqcond (Ignored)",
    )
    grp_model.add_argument(
    ...
    grp_train.add_argument(
        "--use-multiple-tpus",
        action="store_true",
        help="Enable pmap/multi-device training (Ignored)",
    )
    grp_train.add_argument(
        "--fsdp",
        action="store_true",
        dest="full_shard_data_parallel",
        help="Enable FSDP (Ignored)",
    )
    grp_train.add_argument(
        "--freeze-thetas", action="store_true", help="Freeze theta parameters (Ignored)"
    )
    grp_train.add_argument(
        "--batch-size", type=int, default=None, help="Override batch size (Ignored)"
    )
    grp_train.add_argument(
        "--grad-accum-steps",
        type=int,
        default=None,
        help="Number of gradient accumulation steps (Ignored)",
    )
    grp_train.add_argument(
        "--total-steps",
        type=int,
        default=None,
        help="Override total training steps (Ignored)",
    )
    grp_train.add_argument(
        "--save-every-n-steps",
        type=int,
        default=None,
        help="Checkpoint interval (Ignored)",
    )
    grp_train.add_argument(
        "--log-every-n-steps", type=int, default=None, help="Logging interval (Ignored)"
    )
    grp_train.add_argument(
        "--generate-every-n_steps",
        type=int,
        default=None,
        help="Generation sample interval (Ignored)",
    )
    grp_train.add_argument(
        "--wandb-project", type=str, default=None, help="WandB project name (Ignored)"
    )
    grp_train.add_argument(
        "--prefetch-batches",
        type=int,
        default=None,
        help="Number of batches to prefetch (Ignored)",
    )
    grp_train.add_argument(
        "--lr",
        type=float,
        default=1e-3,
        dest="base_lr",
        help="Learning rate (supports scientific notation like 1e-3, 6e-4) (Ignored)",
    )
    grp_train.add_argument(
        "--no-remat",
        action="store_true",
