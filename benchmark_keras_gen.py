...
def benchmark_keras(checkpoint_path, num_tokens=128, warmup=16, batch_sizes=(1, 4)):
    ...
    for B in batch_sizes:
        ...
        if B > 1:
            ...
            logits_np = np.tile(logits_np, (B, 1))
        ...
        print(
            f"  B={B:2d}  prefill={t_prefill:.2f}s ({len(prompt_toks)} toks)  "
...
def benchmark_torch(checkpoint_path, num_tokens=128, warmup=16, batch_sizes=(1, 4)):
    ...
    for B in batch_sizes:
        ...
        print(
            f"  B={B:2d}  decode={elapsed:.2f}s ({num_tokens} steps)  "
            f"{tps:.1f} tok/s  ({tps/B:.1f} tok/s/sample)"
        )
...
def main():
    ...
    parser.add_argument(
        "--num-tokens", type=int, default=128, help="Tokens to generate (default: 128)"
    )
    parser.add_argument(
        "--warmup", type=int, default=16, help="Warmup steps (default: 16)"
    )
    parser.add_argument(
        "--batch-sizes",
        type=int,
    ...
    parser.add_argument(
        "--also-torch",
        action="store_true",
        help="Also benchmark PyTorch+Triton for comparison",
    )
    parser.add_argument(
        "--only-keras", action="store_true", help="Only run Keras benchmark"
    )
    parser.add_argument(
        "--only-torch", action="store_true", help="Only run PyTorch+Triton benchmark"
    )
    ...
    # Run both: use subprocesses to fully free GPU between runs
    if args.also_torch:
        import subprocess
        import sys

        print("\n--- Running Keras benchmark in subprocess ---")
        subprocess.run(
            [
                sys.executable,
                __file__,
                ...
                str(args.warmup),
                "--batch-sizes",
                *[str(b) for b in args.batch_sizes],
                "--only-keras",
            ]
        )
        print("\n--- Running PyTorch+Triton benchmark in subprocess ---")
        subprocess.run(
            [
                sys.executable,
                __file__,
