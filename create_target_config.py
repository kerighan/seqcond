...
def create_target_config_interactive(original_config: ModelConfig) -> ModelConfig:
    ...
    num_kv_heads = get_int_input("num_kv_heads", original_config.num_kv_heads or original_config.num_heads)
...
def create_target_config_preset(
    ...
    return ModelConfig(
        ...
        num_kv_heads=int((original_config.num_kv_heads or original_config.num_heads) * scale),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create target config for model upscaling")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    if "config" in checkpoint:
        original_config = ModelConfig(**checkpoint["config"])
    else:
    ...
    print(f"\nTarget model:")
    print(f"  d_model: {target_config.d_model} ({target_config.d_model/original_config.d_model:.2f}x)")
    print(f"  d_ff: {target_config.d_ff} ({target_config.d_ff/original_config.d_ff:.2f}x)")
    print(f"  num_layers: {target_config.num_layers}")
    print(f"  num_heads: {target_config.num_heads} ({target_config.num_heads/original_config.num_heads:.2f}x)")
    print(f"  seqcond_heads: {target_config.seqcond_heads} ({target_config.seqcond_heads/original_config.seqcond_heads:.2f}x)")
