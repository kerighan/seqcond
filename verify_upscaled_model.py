...
def verify_reconstruction_quality(
    ...
    original_state = original.get("model", original)
    upscaled_state = upscaled.get("model", upscaled)
    ...
    for name, orig_weight in original_state.items():
        ...
        try:
            ...
            if downsampled.shape != orig_weight.shape:
                print(f"Shape mismatch for {name}: {downsampled.shape} vs {orig_weight.shape}")
                continue
    ...
    if reconstruction_errors:
        ...
        avg_rel_error = np.mean([v["relative_error"] for v in reconstruction_errors.values()])
...
def compare_models(
    ...
    print("Consider fine-tuning the upscaled model on a small dataset to verify it learns.")
